...
describe.skipIf(TEST_SKIP)('getByKey', () => {
  ...
  describe('#02 => getByKey.options', () => {
    ...
    describe(`#01 => Config ::: ${JSON.stringify(CONFIG1)}`, () => {
      const { acceptation, success } = createTests.withImplementation(
        dumbFn,
        {
          name: `getByKey.options(${JSON.stringify(CONFIG1)})`,
          instanciation: async () => {
            const func = await import(this1).then(
              ({ getByKey }) => getByKey,
            );

            return func.options(CONFIG1) as any;
          },
        },
      );
    ...
    describe(`#02 => Config ::: ${JSON.stringify(CONFIG2)}`, () => {
      const { acceptation, success } = createTests.withImplementation(
        dumbFn,
        {
          name: `getByKey.options(${JSON.stringify(CONFIG2)})`,
          instanciation: async () => {
            const func = await import(this1).then(
              ({ getByKey }) => getByKey,
            );

            return func.options(CONFIG2) as any;
          },
        },
      );
